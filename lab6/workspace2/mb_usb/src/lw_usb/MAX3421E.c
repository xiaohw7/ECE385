...
void MAXreg_wr(BYTE reg, BYTE val) {
    ...
    // Assume XSpi_Transfer sends and then receives data simultaneously
    int St = XSpi_Transfer(&SpiInstance, &wr, &rd, 2);
    if (St != 0) {
        xil_printf("Error:  %d\r\n", St);
    }
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int St = XSpi_Transfer(&SpiInstance, &val, &rd, 2+nbytes);
	if (St != 0) {
        xil_printf("Error:  %d\r\n", St);
    }
...
BYTE MAXreg_rd(BYTE reg) {
    ...
    // Assume XSpi_Transfer sends and then receives data simultaneously
    int St = XSpi_Transfer(&SpiInstance, &reg, &rval, 2);
    if (St != 0) {
        xil_printf("Error:  %d\r\n", St);
    }
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
    ...
    // Assume XSpi_Transfer sends and then receives data simultaneously
    int St = XSpi_Transfer(&SpiInstance, &reg, data, 1+nbytes);
    if (St != 0) {
        xil_printf("Error:  %d\r\n", Status);
    }
