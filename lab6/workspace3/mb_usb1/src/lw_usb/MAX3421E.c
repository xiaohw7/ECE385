...
//    BYTE rd[2] = {0};
    ...
    int St = XSpi_Transfer(&SpiInstance, wr, 0, 2);
    if (St != 0) {
        xil_printf("Error:  %d\r\n", St);
    }
...
//	BYTE rd[2] = {0};
	...
	int St = XSpi_Transfer(&SpiInstance, val, 0, 1+nbytes);
	if (St != 0) {
        xil_printf("Error:  %d\r\n", St);
    }
...
BYTE MAXreg_rd(BYTE reg) {
    ...
    // Assume XSpi_Transfer sends and then receives data simultaneously
    int St = XSpi_Transfer(&SpiInstance, wr, rval, 2);
    if (St != 0) {
        xil_printf("Error:  %d\r\n", St);
    }
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
    ...
    // Assume XSpi_Transfer sends and then receives data simultaneously
    int St = XSpi_Transfer(&SpiInstance, wr, rd, 1+nbytes);
    if (St != 0) {
        xil_printf("Error:  %d\r\n", Status);
    }
